...
level: a1
order: 2
title: 〜時半、〜時15分、〜分前 (eta erdiak, eta laurden, gutxi)
estimatedMinutes: 12
covers:
- time-detail
- time-basics
...
- id: ex-02-ee-mc1
  ...
  - Bost erdi
  answer: 1
  explanation: 「〜時半」には「eta erdiak」（文字どおりには「と半分（複数形）」）を使います。「eta erdiak」の前では数字は「-ak」をつけません：「bost eta erdiak」。
- id: ex-02-ee-mc2
  type: multiple-choice
  prompt: 3時15分です。どう言いますか。
  options:
  - Hiruak eta laurden
  ...
  - Hiru eta laurdenak
  answer: 2
  explanation: 「eta laurden」は「15分過ぎ（と4分の1）」という意味で、時刻は複数形（「hirurak」）になります：*Hirurak eta laurden dira*。
- id: ex-02-ee-mc3
  type: multiple-choice
  prompt: 9時15分前です。どう言いますか。
  options:
  ...
  answer: 0
  explanation: バスク語では「〜分前」は「gutxi」と言い、後ろに置きます。直訳すると「9時、15分、足りない」となります。
- id: ex-02-ee-fb1
  type: fill-in-blank
  prompt: 7時5分です = Zazpiak eta ___ dira.
  answers:
  - bost
  explanation: 過ぎた分は「eta + 数字」で表します。5 = bost。
- id: ex-02-ee-fb2
  type: fill-in-blank
  prompt: 4時10分前です = Laurak hamar ___ dira.
  answers:
  - gutxi
  explanation: 時刻の表現では「gutxi」は「〜前（足りない）」という意味です。
- id: ex-02-ee-fc
  type: flashcards
  cards:
  - eu: eta erdiak
    es: 〜時半（3:30 = hiru eta erdiak）
  - eu: eta laurden
    es: 〜時15分（3:15 = hirurak eta laurden）
  - eu: laurden gutxi
    es: 〜時15分前（2:45 = hirurak laurden gutxi）
  - eu: gutxi
    es: 〜分前（時刻の表現で）
...
バスク語で時刻の「分」を表すには、ほぼ全てをカバーする3つの言い方があります：**eta erdiak**（〜時半）、**eta laurden**（〜時15分）、**gutxi**（〜分前）。コツは、それぞれをどこに置くかを知ることです。
...
> 注目：*eta erdiak* を使う場合、その前の数字には **-ak をつけません**（*bostak* ではなく *bost*）。複数形の意味は *erdiak* がすでに担っているからです。
...
30分を過ぎたあとは、次の時刻を基準にして **gutxi**（「足りない、〜分前」）という単語で言います。語順は：**時刻-ak + 分 + gutxi**。
...
> スペイン語の *menos cuarto*（15分前）とは違い、バスク語では **gutxi** が数字の**後ろ**に来るのが珍しい点です。*Bederatziak laurden gutxi* は直訳すると「9時、15分、足りない」となります。
