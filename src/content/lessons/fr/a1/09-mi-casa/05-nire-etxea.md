---
id: a1-home-5
unitId: a1-home
code: 05-nire-etxea
unit: 09-mi-casa
level: a1
order: 5
title: Nire etxea — Décrire sa propre maison
estimatedMinutes: 12
covers: [house-description, home-rooms, furniture, house-chores, companions-rekin]
exercises:
  - id: ex-09-05-mc1
    type: multiple-choice
    prompt: 'Pour demander « Combien de pièces a-t-elle ? » tu dis…'
    options: ["Non dago etxea?", "Zenbat gela ditu?", "Norekin bizi zara?", "Zer egiten duzu?"]
    answer: 1
    explanation: '« Zenbat » = combien, « gela » = pièce, « ditu » = elle a (plusieurs objets).'
  - id: ex-09-05-fb1
    type: fill-in-blank
    prompt: 'Ma maison est lumineuse = Nire etxea ___ da.'
    answers: [argitsua]
    explanation: '« Argitsua » = lumineux/lumineuse (de « argi » = lumière + suffixe -tsu d''abondance).'
  - id: ex-09-05-mc2
    type: multiple-choice
    prompt: '« Etxea hirugarren solairuan dago » signifie...'
    options: [La maison a trois pièces, La maison est au troisième étage, La maison a trois fenêtres, La maison coûte trois mille]
    answer: 1
    explanation: '« Hirugarren » = troisième, « solairua » = étage, « dago » = est. « Elle est au troisième étage ».'
  - id: ex-09-05-mp
    type: match-pairs
    pairs:
      - { eu: argitsua, es: lumineux/lumineuse }
      - { eu: handia, es: grand/grande }
      - { eu: txikia, es: petit/petite }
      - { eu: lasaia, es: calme }
      - { eu: berria, es: neuf/neuve }
      - { eu: zaharra, es: vieux/vieille }
  - id: ex-09-05-fc
    type: flashcards
    cards:
      - { eu: 'Nolakoa da zure etxea?', es: 'Comment est ta maison ?' }
      - { eu: argitsua, es: lumineux/lumineuse }
      - { eu: iluna, es: sombre }
      - { eu: handia, es: grand/grande }
      - { eu: txikia, es: petit/petite }
      - { eu: lasaia, es: calme }
      - { eu: zaratatsua, es: bruyant/bruyante }
      - { eu: berria, es: neuf/neuve }
      - { eu: zaharra, es: vieux/vieille }
      - { eu: erosoa, es: confortable }
      - { eu: 'Non dago zure etxea?', es: 'Où est ta maison ?' }
      - { eu: erdigunean, es: au centre }
      - { eu: auzo lasai batean, es: dans un quartier calme }
      - { eu: hirugarren solairuan, es: au troisième étage }
      - { eu: 'Etxeak badu igogailua?', es: 'La maison a-t-elle un ascenseur ?' }
      - { eu: 'Norekin bizi zara?', es: 'Avec qui vis-tu ?' }
---

Cette leçon intègre tout ce qui concerne le thème 9 : les pièces, les meubles, les tâches et avec qui tu vis. L'objectif est que tu puisses **décrire ta maison** en phrases simples, comme si on t'interrogeait en classe ou dans une conversation informelle.

## Les questions de base

| Question | Français |
|---|---|
| *Non dago zure etxea?* | Où est ta maison ? |
| *Nolakoa da?* | Comment est-elle ? |
| *Zenbat gela ditu?* | Combien de pièces a-t-elle ? |
| *Norekin bizi zara?* | Avec qui vis-tu ? |
| *Zenbatgarren solairuan bizi zara?* | À quel étage vis-tu ? |
| *Etxeak badu igogailua / balkoia / terraza?* | La maison a-t-elle un ascenseur / un balcon / une terrasse ? |

## Adjectifs utiles pour décrire la maison

| Euskera | Français |
|---|---|
| *handia* | grande |
| *txikia* | petite |
| *argitsua* | lumineuse |
| *iluna* | sombre |
| *berria* | neuve |
| *zaharra* | vieille |
| *lasaia* | calme |
| *zaratatsua* | bruyante |
| *erosoa* | confortable |
| *garestia* | chère |
| *merkea* | bon marché |

> Souviens-toi du schéma **NOLAKOA da?** déjà vu dans d'autres unités : la réponse porte l'adjectif + `-a` final + verbe *izan*. Ex. : *Nire etxea **handia da**.* — Ma maison est grande.

## Un paragraphe modèle

Voici un texte simple décrivant une maison. Lis-le à voix haute et observe les schémas.

> *Ni Iruñean bizi naiz, auzo lasai batean. Nire etxea ez da oso handia: hiru gela ditu — bi logela eta egongela bat — sukaldea eta komuna. Egongela argitsua da, eta balkoi bat dauka. Hirugarren solairuan dago, eta etxeak igogailua du.*
>
> *Bikotekidearekin bizi naiz. Berak bazkaria prestatzen du, eta nik ontziak garbitzen ditut. Asteburuetan biok elkarrekin egiten dugu erosketa.*

## À ton tour : décris ta maison

Construis ton propre paragraphe en répondant dans cet ordre :

1. **Non bizi zara?** (Où vis-tu ?) → *X-(e)an bizi naiz.*
2. **Nolakoa da etxea?** (Comment est-elle ?) → *Nire etxea handia / argitsua / lasaia da.*
3. **Zenbat gela ditu?** (Combien de pièces ?) → *Bi / Hiru / Lau gela ditu.*
4. **Zer dago etxean?** (Qu'y a-t-il ?) → *Sukaldea, egongela, bi logela eta komun bat ditu.*
5. **Norekin bizi zara?** → *Bakarrik / Gurasoekin / Lagunekin bizi naiz.*
6. **Etxeko lanak nork egiten ditu?** → *Nik egunero garbitzen dut* / *Pisukideen artean banatzen ditugu.*

## Vocabulaire de clôture

| Euskera | Français |
|---|---|
| *etxebizitza* | logement |
| *etxejabea* | propriétaire |
| *alokairua* | location |
| *errenta* | loyer mensuel |
| *auzoa* | quartier |
| *erdigunea* | centre (de la ville) |
| *aldiriak* | périphérie / banlieue |

> **Conseil :** quand tu parles de ta maison en classe, n'essaie pas de traduire chaque détail. Tiens-t'en à des phrases courtes et bien construites. *"Bi logela ditut, sukalde txiki bat eta egongela handia"* est une meilleure description qu'une longue phrase pleine de fautes.
</content>
</invoke>
