---
id: a1-week-plan-5
unitId: a1-week-plan
code: 05-nire-astea
unit: 13-agenda
level: a1
order: 5
title: 我的一周（综合课）
estimatedMinutes: 12
covers: [weekly-planning, future-koGo, time-when, days-of-week, common-verbs]
exercises:
  - id: ex-nast-mc1
    type: multiple-choice
    prompt: '"明天我要去健身房" =…'
    options: ['Bihar kiroldegira joan naiz', 'Bihar kiroldegira joango naiz', 'Bihar kiroldegian nago', 'Kiroldegira bihar dut']
    answer: 1
    explanation: '将来时：*joan + go = joango* + *naiz*（我）。*Kiroldegira* = 去健身房（表示目的地的后缀 -ra）。'
  - id: ex-nast-mc2
    type: multiple-choice
    prompt: '"星期六我要和 Maialen 约着去喝咖啡" =…'
    options: ['Larunbatean Maialenekin geratuko naiz kafea hartzeko', 'Larunbat Maialen kafea hartu', 'Maialen larunbatean kafea du', 'Larunbatean kafea Maialenekin']
    answer: 0
    explanation: '*Larunbatean*（星期六）+ *Maialenekin*（和 Maialen）+ *geratuko naiz*（我会约见）+ *kafea hartzeko*（去喝咖啡）。'
  - id: ex-nast-fb1
    type: fill-in-blank
    prompt: '"下周我会有考试" = ___ astean azterketak izango ditut.'
    answers: [Datorren]
    explanation: '*Datorren* —— "即将到来的"。*Datorren astean* —— "下周"。'
  - id: ex-nast-fb2
    type: fill-in-blank
    prompt: '"星期五我要和朋友们出去" = Ostiralean lagunekin atera___ naiz.'
    answers: [ko]
    explanation: '*Atera* + *ko* + *naiz* = *aterako naiz*（我会出去）。以 -a 结尾的动词 → -ko。'
  - id: ex-nast-fc
    type: flashcards
    cards:
      - { eu: agenda, es: 日程表 }
      - { eu: 'Asteko agenda', es: 一周的日程 }
      - { eu: 'Zer egingo duzu astelehenean?', es: 你星期一要做什么？ }
      - { eu: 'Kiroldegira joango naiz', es: 我要去健身房 }
      - { eu: 'Lagunekin afaltzera joango gara', es: 我们要和朋友们去吃晚饭 }
      - { eu: 'Asteburu honetan mendira joango gara', es: 这个周末我们要去山里 }
      - { eu: 'Datorren astean lan asko izango dut', es: 下周我会有很多工作 }
      - { eu: 'Ostiralean parranda egingo dugu', es: 星期五我们要出去玩 }
      - { eu: 'Bilera dut hamarretan', es: 我十点有个会议 }
      - { eu: 'Libre zaude?', es: '你有空吗？' }
      - { eu: 'Geratuko gara?', es: '我们约一下吗？' }
  - id: ex-nast-mp
    type: match-pairs
    pairs:
      - { eu: 'datorren astean', es: 下周 }
      - { eu: 'asteburuan', es: 周末 }
      - { eu: 'bihar', es: 明天 }
      - { eu: 'etzi', es: 后天 }
      - { eu: 'gaur arratsaldean', es: 今天下午 }
      - { eu: 'libre nago', es: 我有空 }
---

本单元的最后一课：我们要把学过的所有内容串起来，安排**一整周**。星期、将来时、约会、表示时间的标记词……这就是做计划的语言。

## 开放式问题

> *Zer egingo duzu aste honetan?* —— 你这周打算做什么？

一个自然的回答会用 星期 + 将来时：

> *Astelehenean kiroldegira joango naiz. Asteartean euskara klasea dut. Asteazkenean bilera bat izango dut. Ostegunean lasai egongo naiz. Ostiralean lagunekin afaltzera joango gara. Asteburuan mendira joango gara.*
>
> 星期一我要去健身房。星期二我有巴斯克语课。星期三我会有个会议。星期四我会很轻松。星期五我们要和朋友们去吃晚饭。周末我们要去山里。

## 基本模板

> **星期 + 活动 + 将来时**

| 星期 | 活动示例 |
|---|---|
| *Astelehenean...* | *...kiroldegira joango naiz* |
| *Asteartean...* | *...lan egingo dut* |
| *Asteazkenean...* | *...amarekin geratuko naiz* |
| *Ostegunean...* | *...filma bat ikusiko dut* |
| *Ostiralean...* | *...lagunekin aterako naiz* |
| *Larunbatean...* | *...mendira joango gara* |
| *Igandean...* | *...etxean lasai egongo naiz* |

## 日程中常用的动词

| 动词 | 将来时形式 | 中文 |
|---|---|---|
| joan | joango naiz/da | 我去、他去 |
| etorri | etorriko naiz/da | 我来、他来 |
| egon | egongo naiz | 我会在/会待着 |
| izan | izango da | 会是 / 会有 |
| ikusi | ikusiko dut | 我会看（它） |
| egin | egingo dut | 我会做（它） |
| jan | jango dut | 我会吃（它） |
| lan egin | lan egingo dut | 我会工作 |
| atera | aterako naiz | 我会出去 |
| eraman | eramango dut | 我会带（它） |
| erosi | erosiko dut | 我会买（它） |

## 小故事：Iratxe 的一周

读读这段虚构的日程，注意其中表示时间的标记词：

> *Astelehenean lana dut goizean, eta arratsaldean kiroldegira joango naiz. Asteartean medikuarekin daukat hitzordua, hamabietan. Asteazkenean ez dut ezer berezirik, etxean afalduko dut lasai. Ostegunean Maialenekin geratuko naiz kafe bat hartzeko, sei eta erdietan. Ostiralean afari bat dugu lankideekin. Larunbatean asteburu pasa: Bilbora joango gara nire bikotekidearekin. Igandean buelta etxera, eta arratsaldean lasai-lasai filma bat ikusiko dut.*

## 轮到你了：三个示范回答

**问题：** *Bihar zer egingo duzu?*

> *Bihar lanera joango naiz goizean. Arratsaldean kafea hartuko dut lagun batekin, eta gauean etxean afalduko dut.*

**问题：** *Datorren astean zer plan duzu?*

> *Datorren astean azterketa bat izango dut. Asko ikasiko dut. Larunbatean lasai gelditu nahi dut.*

**问题：** *Asteburu honetan zer egingo duzu?*

> *Asteburu honetan mendira joango gara lagunekin. Larunbatean Aralarrera joango gara, eta igandean lasai geldituko gara etxean.*

## 安排一周的关键短语

- **Libre zaude X-(e)an?** —— 你在 X 那天有空吗？
- **Hamarretan ondo?** —— 十点可以吗？
- **Beste egun batean izan beharko du.** —— 得改到另一天了。
- **Konforme, X-(e)an geratuko gara.** —— 好的，我们 X 那天约。

## 图示复习

```
Pasado    →  Presente  →  Futuro
atzo         gaur          bihar — etzi — datorren astean
joan zen     doa            joango da
ikusi dut    ikusten dut    ikusiko dut
```

> **文化小贴士：** 在很多地方你会看到诸如 *"Asteburu honetan jaiak"* 或 *"Datorren ostiralean kontzertua"* 这样的告示。像 *datorren* 和 *asteburu honetan* 这样的时间标记词，既出现在日常对话中，也出现在街区里的各种信息看板上。一旦耳朵能辨认出它们，看懂街上的告示就会变得容易得多。
