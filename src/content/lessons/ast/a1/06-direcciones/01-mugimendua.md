---
id: a1-directions-1
unitId: a1-directions
code: 01-mugimendua
unit: 06-direcciones
level: a1
order: 1
title: Verbos de movimientu (joan, etorri, ibili)
estimatedMinutes: 12
covers:
- movement-verbs
- common-verbs
exercises:
- id: ex-mu-mp
  type: match-pairs
  pairs:
  - eu: joan
    es: ir
  - eu: etorri
    es: venir
  - eu: ibili
    es: andar / moverse
  - eu: sartu
    es: entrar
  - eu: irten
    es: salir
  - eu: igo
    es: subir
  - eu: jaitsi
    es: bajar
- id: ex-mu-fb1
  type: fill-in-blank
  prompt: Yo voi = Ni ___.
  answers:
  - noa
  explanation: La forma sintética de "joan" pa "yo" ye "noa". Frecuente n'entrugues como "Nora noa?" → "¿Aú voi?".
- id: ex-mu-fb2
  type: fill-in-blank
  prompt: Tú vienes = Zu ___.
  answers:
  - zatoz
  explanation: '"Zatoz" ye la forma sintética de "etorri" pa "zu".'
- id: ex-mu-mc1
  type: multiple-choice
  prompt: '"Nora zoaz?" significa…'
  options:
  - ¿D'aú vienes?
  - ¿Aú vas?
  - ¿Aú tas?
  - ¿Per aú andes?
  answer: 1
  explanation: '"Nora" = aú; "zoaz" = vas (forma sintética de "joan" pa "zu").'
- id: ex-mu-mc2
  type: multiple-choice
  prompt: '"Sartu" y el so contrariu "irten" signifiquen, respectivamente:'
  options:
  - entrar / salir
  - subir / bajar
  - ir / venir
  - abrir / cerrar
  answer: 0
- id: ex-mu-fc
  type: flashcards
  cards:
  - eu: joan
    es: dir
  - eu: noa
    es: voi
  - eu: zoaz
    es: vas
  - eu: doa
    es: va
  - eu: goaz
    es: vamos
  - eu: zoazte
    es: vais
  - eu: doaz
    es: van
  - eu: etorri
    es: venir
  - eu: nator
    es: vengo
  - eu: zatoz
    es: vienes
  - eu: dator
    es: vien
  - eu: gatoz
    es: venimos
  - eu: zatozte
    es: venís
  - eu: datoz
    es: vienen
  - eu: ibili
    es: andar / tar (andando d'un llau pa otru)
  - eu: nabil
    es: ando
  - eu: zabiltza
    es: andes
  - eu: dabil
    es: anda
  - eu: sartu
    es: entrar
  - eu: irten
    es: salir
  - eu: igo
    es: xubir
  - eu: jaitsi
    es: baxar
---

Trés verbos básicos van apaecer tol tiempu: **joan** (dir), **etorri** (venir) ya **ibili** (andar, "movese d'un llau pa otru"). Los trés tienen formes **sintétiques** (ensin auxiliar) que s'usen muncho en presente — y convien deprendeles como bloque.

## Joan — dir

| Persona | Forma | Asturianu |
|---|---|---|
| ni | **noa** | voi |
| zu | **zoaz** | vas |
| hura | **doa** | va |
| gu | **goaz** | vamos |
| zuek | **zoazte** | vais |
| haiek | **doaz** | van |

> La entruga clave: ***Nora zoaz?*** — ¿Aú vas? Trabayarémosla na lleición 03.

## Etorri — venir

| Persona | Forma | Asturianu |
|---|---|---|
| ni | **nator** | vengo |
| zu | **zatoz** | vienes |
| hura | **dator** | vien |
| gu | **gatoz** | venimos |
| zuek | **zatozte** | venís |
| haiek | **datoz** | vienen |

> La entruga clave: ***Nondik zatoz?*** — ¿D'aú vienes? Trabayarémosla na lleición 04.

## Ibili — andar, movese

| Persona | Forma | Asturianu |
|---|---|---|
| ni | **nabil** | ando, voi moviéndome |
| zu | **zabiltza** | andes |
| hura | **dabil** | anda |
| gu | **gabiltza** | andamos |
| zuek | **zabiltzate** | andáis |
| haiek | **dabiltza** | anden |

> *Ibili* val tamién pa entrugar "qué andes faciendo": **Zer zabiltza?** — ¿Qué tal? / ¿Qué traes ente manes? Ye perfrecuente como saludu ente persones que se conocen.

## Otros verbos de movimientu útiles

| Euskera | Asturianu | Exemplu |
|---|---|---|
| *sartu* | entrar | *Tabernan sartu naiz.* — Entré nel chigre. |
| *irten* | salir | *Etxetik irten naiz.* — Salí de casa. |
| *igo* | xubir | *Eskailerak igo ditut.* — Xubí les escaleres. |
| *jaitsi* | baxar | *Aldapa jaitsi dut.* — Baxé la cuesta. |
| *iritsi / heldu* | llegar | *Iruñera iritsi naiz.* — Llegué a Iruña. |

## Por qué importen les formes sintétiques

En euskera, la mayoría de los verbos conxúguense con un **auxiliar**: *jan dut* (comí), *ikusi dut* (vi). Pero unos pocos verbos perfrecuentes — ente ellos **joan**, **etorri**, **egon**, **ibili**, **eduki**, **jakin** — tienen una conxugación propia "compacta" (*sintética*) que s'usa en presente. Como apaecen muncho, ye más fácil memorizar les seis formes que dar mil arrodeos.

## Mini-conversación

> — *Aupa, nora zoaz?* — ¡Hola! ¿Aú vas?
>
> — *Banketxera **noa**. Eta zu, nondik **zatoz**?* — Voi al bancu. ¿Y tú, d'aú vienes?
>
> — *Lanetik **nator**.* — Vengo del trabayu.
>
> — *Eta Lide? Zer **dabil**?* — ¿Y Lide? ¿Qué fai?
>
> — *Enkargutan **dabil**, batetik bestera.* — Anda faciendo recaos, d'un llau pa otru.

> **Nota:** les formes sintétiques son un lladriyu central d'A1. Memorízales en bloques de 2 (ni/zu) o de 3 (gu/zuek/haiek) cantando, escribiendo, repitiendo. La inversión vendrá sola cola práctica.
