---
id: a1-routine-5
unitId: a1-routine
code: 05-nire-eguna
unit: 07-rutina-diaria
level: a1
order: 5
title: El mio día (cuenta la to rutina)
estimatedMinutes: 12
covers:
- daily-routine
- habitual-aspect
- time-basics
- time-detail
- parts-of-day
- frequency-adverbs
exercises:
- id: ex-05-ne-mc1
  type: multiple-choice
  prompt: '"Zer ordutan esnatzen zara?" significa…'
  options:
  - ¿Qué hora ye?
  - ¿A qué hora te despiertes?
  - ¿Cuándo te acuestes?
  - ¿Qué desayunes?
  answer: 1
  explanation: '"Zer ordutan…?" = "¿a qué hora…?". El sufixu "-tan" funciona como "a les (hores)".'
- id: ex-05-ne-mc2
  type: multiple-choice
  prompt: ¿Cómo diríes "A les siete despiértome"?
  options:
  - Zazpiak esnatzen naiz
  - Zazpietan esnatzen naiz
  - Zazpiak dira esnatzen
  - Zazpiko esnatu naiz
  answer: 1
  explanation: 'Pa dicir "a les (hores)" úsase "-etan" sobre la hora plural: zazpi**etan** = a les siete.'
- id: ex-05-ne-fb1
  type: fill-in-blank
  prompt: A les nueve = ___ (enllena la pallabra completa).
  answers:
  - bederatzietan
  - Bederatzietan
  explanation: 'La hora "a les X" fórmase con "-(e)tan": *bederatzi* + *-etan* = *bederatzietan*.'
- id: ex-05-ne-fb2
  type: fill-in-blank
  prompt: Pela mañana voi al trabayu = Goizean ___ joaten naiz.
  answers:
  - lanera
  - Lanera
  explanation: '"Lan" = trabayu. Col casu NORA (-ra, direición): *lanera* = "al trabayu".'
- id: ex-05-ne-fc
  type: flashcards
  cards:
  - eu: Zer ordutan esnatzen zara?
    es: ¿A qué hora te despiertes?
  - eu: Zer ordutan oheratzen zara?
    es: ¿A qué hora te acuestes?
  - eu: Zer egiten duzu goizean?
    es: ¿Qué faes pela mañana?
  - eu: zazpietan
    es: a les siete
  - eu: zortzietan
    es: a les ocho
  - eu: hamarretan
    es: a les diez
  - eu: hamabietan
    es: a les doce
  - eu: ordu bietan
    es: a les dos (en puntu)
  - eu: bost eta erdietan
    es: a les cinco y media
  - eu: lanera
    es: al trabayu
  - eu: euskaltegira
    es: al euskaltegi (escuela d'euskera)
  - eu: etxera
    es: a casa
  - eu: ohera
    es: a la cama
---

Esta lleición xunta too lo de la unidá: hora, momentos del día, verbos de rutina y frecuencia. L'oxetivu ye que puedas contar el to día completu n'euskera de principiu a fin.

## La entruga clave: "Zer ordutan…?"

| Entruga | Asturianu |
|---|---|
| *Zer ordutan esnatzen zara?* | ¿A qué hora te despiertes? |
| *Zer ordutan jaikitzen zara?* | ¿A qué hora te llevantes? |
| *Zer ordutan gosaltzen duzu?* | ¿A qué hora desayunes? |
| *Zer ordutan bazkaltzen duzu?* | ¿A qué hora comes? |
| *Zer ordutan ateratzen zara etxetik?* | ¿A qué hora sales de casa? |
| *Zer ordutan afaltzen duzu?* | ¿A qué hora cenes? |
| *Zer ordutan oheratzen zara?* | ¿A qué hora te acuestes? |

> Güeyu al cambiu d'auxiliar: *esnatzen **zara*** (NOR), pero *gosaltzen **duzu*** (NOR-NORK). Toca que te vayas fixando na pareya verbu + auxiliar.

## "A les X" — cómo dicilo

Pa responder "a les (hores)", cueyi la hora en plural y camuda *-ak* por **-etan** (con `-e-` intercalada sacante nes que yá acaben en vocal):

| Hora | "A les…" |
|---|---|
| zazpiak (les siete) | **zazpietan** |
| zortziak | **zortzietan** |
| hamarrak | **hamarretan** |
| hamabiak | **hamabietan** |
| ordu bata (la una) | **ordu batean** (singular, caltién la so forma) |
| ordu biak | **ordu bietan** |
| bost eta erdiak | **bost eta erdietan** (a les cinco y media) |
| zortziak laurden gutxi | **zortziak laurden gutxitan** (a les ocho menos cuartu) |

## Un día completu — modelu

Llei esti testu d'exemplu y fíxate en cómo s'enllacen toles pieces:

> *Ni goizeko zazpietan esnatzen naiz. Poliki-poliki altxatzen naiz eta komunera joaten naiz. Dutxa bat hartzen dut, arropa janzten dut eta sukaldera joaten naiz. Sukaldean gosaria prestatzen dut eta lasai-lasai gosaltzen dut. Ondoren, etxetik ateratzen naiz eta lanera joaten naiz. Eguerdiko ordu bietan bazkaltzen dut. Arratsaldean batzuetan zinemara joaten naiz, beste batzuetan lagunekin egoten naiz. Gauean, etxean afaltzen dut, telebista ikusten dut eta hamaiketan oheratzen naiz.*

Traducción averada:
> *Despiértome a les siete de la mañana. Llevántome adulces y voi al bañu. Dúchome, vístome y voi a la cocina. Na cocina preparo'l desayunu y desayuno con calma. Dempués salgo de casa y voi al trabayu. A les dos del mediudía como. Pela tarde a vegaes voi al cine, otres vegaes toi colos amigos. Pela nueche, ceno en casa, veo la tele y acuéstome a les once.*

## Construye'l tuyu

Sigui esta plantía cola to información real. Ye la meyor manera de fixar lo deprendío:

1. **A qué hora te despiertes:** *Goizeko ___etan esnatzen naiz.*
2. **Qué faes al llevantate:** *... dutxatzen naiz / gosaltzen dut / ...*
3. **Cómo vas al trabayu o estudiu:** *... oinez / autoz / autobusez joaten naiz.*
4. **Qué faes pela tarde:** *Arratsaldean ...*
5. **A qué hora cenes y te acuestes:** *Gaueko ___etan afaltzen dut eta ___etan oheratzen naiz.*

## Frases útiles pa enllazar

- *Lehenengo* — primero
- *Gero / Ondoren* — llueu, dempués
- *Eta gero* — y llueu
- *Azkenik* — finalmente
- *Egunero* — tolos díes
- *Batzuetan / Beste batzuetan* — a vegaes / otres vegaes
- *Beti* — siempre
- *Inoiz ez* — nunca

> **Nota cultural:** n'Euskal Herria muncha xente come (*bazkaldu*) ente les 14:00 y les 15:00 y cena (*afaldu*) bien sero, sobre les 21:00-22:00. *Hamaiketakoa* — l'almuerzu de les once de la mañana, polo xeneral un pintxo o bocadiellu — ye una institución. Si vas a una cuadrilla y a les 11h daquién diz *hamaiketakoa egitera goaz*, yá sabes lo que toca.
