---
id: a1-week-plan-5
unitId: a1-week-plan
code: 05-nire-astea
unit: 13-agenda
level: a1
order: 5
title: A mía semana (lección integradora)
estimatedMinutes: 12
covers:
- weekly-planning
- future-koGo
- time-when
- days-of-week
- common-verbs
exercises:
- id: ex-nast-mc1
  type: multiple-choice
  prompt: '"Maitín iré a o chymnasio" =…'
  options:
  - Bihar kiroldegira joan naiz
  - Bihar kiroldegira joango naiz
  - Bihar kiroldegian nago
  - Kiroldegira bihar dut
  answer: 1
  explanation: 'Futuro: *joan + go = joango* + *naiz* (yo). *Kiroldegira* = a o chymnasio (sufixo -ra de destín).'
- id: ex-nast-mc2
  type: multiple-choice
  prompt: '"O sabado quedaré con Maialen ta fer un café" =…'
  options:
  - Larunbatean Maialenekin geratuko naiz kafea hartzeko
  - Larunbat Maialen kafea hartu
  - Maialen larunbatean kafea du
  - Larunbatean kafea Maialenekin
  answer: 0
  explanation: '*Larunbatean* (o sabado) + *Maialenekin* (con Maialen) + *geratuko naiz* (quedaré) + *kafea hartzeko* (ta fer un café).'
- id: ex-nast-fb1
  type: fill-in-blank
  prompt: '"A semana que viene tendré examens" = ___ astean azterketak izango ditut.'
  answers:
  - Datorren
  explanation: '*Datorren* — "que viene". *Datorren astean* — "a semana que viene".'
- id: ex-nast-fb2
  type: fill-in-blank
  prompt: '"O viernes saldré con os amigos" = Ostiralean lagunekin atera___ naiz.'
  answers:
  - ko
  explanation: '*Atera* + *ko* + *naiz* = *aterako naiz* (saldré). Verbo en -a → -ko.'
- id: ex-nast-fc
  type: flashcards
  cards:
  - eu: agenda
    es: achenda
  - eu: Asteko agenda
    es: Achenda d'a semana
  - eu: Zer egingo duzu astelehenean?
    es: Qué ferás o lunes?
  - eu: Kiroldegira joango naiz
    es: Iré a o chymnasio
  - eu: Lagunekin afaltzera joango gara
    es: Iremos a cenar con os amigos
  - eu: Asteburu honetan mendira joango gara
    es: Ista fin de semana iremos a o mont
  - eu: Datorren astean lan asko izango dut
    es: A semana que viene tendré muito treballo
  - eu: Ostiralean parranda egingo dugu
    es: O viernes saldremos de fiesta
  - eu: Bilera dut hamarretan
    es: Tiengo reunión a as diez
  - eu: Libre zaude?
    es: Yes libre?
  - eu: Geratuko gara?
    es: Quedamos?
- id: ex-nast-mp
  type: match-pairs
  pairs:
  - eu: datorren astean
    es: a semana que viene
  - eu: asteburuan
    es: a fin de semana
  - eu: bihar
    es: mañana
  - eu: etzi
    es: pasado mañana
  - eu: gaur arratsaldean
    es: esta tarde
  - eu: libre nago
    es: estoy libre
---

Lección final d'a unidat: imos a montar **una semana entera** unindo tot lo aprendiu. Días, futuros, citas, marcadors de cuan... o lenguache de planificar.

## Pregunta ubierta

> *Zer egingo duzu aste honetan?* — Qué ferás ista semana?

Una respuesta natural usaría o día + futuro:

> *Astelehenean kiroldegira joango naiz. Asteartean euskara klasea dut. Asteazkenean bilera bat izango dut. Ostegunean lasai egongo naiz. Ostiralean lagunekin afaltzera joango gara. Asteburuan mendira joango gara.*
>
> O lunes iré a o chymnasio. O martes tiengo clase de basco. O miércoles tendré una reunión. O chueves estaré tranquilo. O viernes iremos a cenar con os amigos. A fin de semana iremos a o mont.

## Plantilla basica

> **DÍA + ACTIVIDAT + FUTURO**

| Día | Actividat eixemplo |
|---|---|
| *Astelehenean...* | *...kiroldegira joango naiz* |
| *Asteartean...* | *...lan egingo dut* |
| *Asteazkenean...* | *...amarekin geratuko naiz* |
| *Ostegunean...* | *...filma bat ikusiko dut* |
| *Ostiralean...* | *...lagunekin aterako naiz* |
| *Larunbatean...* | *...mendira joango gara* |
| *Igandean...* | *...etxean lasai egongo naiz* |

## Verbos útils ta l'achenda

| Verbo | Forma futura | Aragonés |
|---|---|---|
| joan | joango naiz/da | iré, irá |
| etorri | etorriko naiz/da | vendré, vendrá |
| egon | egongo naiz | estaré |
| izan | izango da | será / i habrá |
| ikusi | ikusiko dut | (lo) veyeré |
| egin | egingo dut | (lo) feré |
| jan | jango dut | (lo) mincharé |
| lan egin | lan egingo dut | treballaré |
| atera | aterako naiz | saldré |
| eraman | eramango dut | (lo) levaré |
| erosi | erosiko dut | (lo) mercaré |

## Mini-historia: a semana d'Iratxe

Leye ista achenda inventada y para cuenta en os marcadors temporals:

> *Astelehenean lana dut goizean, eta arratsaldean kiroldegira joango naiz. Asteartean medikuarekin daukat hitzordua, hamabietan. Asteazkenean ez dut ezer berezirik, etxean afalduko dut lasai. Ostegunean Maialenekin geratuko naiz kafe bat hartzeko, sei eta erdietan. Ostiralean afari bat dugu lankideekin. Larunbatean asteburu pasa: Bilbora joango gara nire bikotekidearekin. Igandean buelta etxera, eta arratsaldean lasai-lasai filma bat ikusiko dut.*

## O tuyo turno: tres respuestas modelo

**Pregunta:** *Bihar zer egingo duzu?*

> *Bihar lanera joango naiz goizean. Arratsaldean kafea hartuko dut lagun batekin, eta gauean etxean afalduko dut.*

**Pregunta:** *Datorren astean zer plan duzu?*

> *Datorren astean azterketa bat izango dut. Asko ikasiko dut. Larunbatean lasai gelditu nahi dut.*

**Pregunta:** *Asteburu honetan zer egingo duzu?*

> *Asteburu honetan mendira joango gara lagunekin. Larunbatean Aralarrera joango gara, eta igandean lasai geldituko gara etxean.*

## Frases clau ta coordinar a semana

- **Libre zaude X-(e)an?** — Yes libre o X?
- **Hamarretan ondo?** — A as diez bien?
- **Beste egun batean izan beharko du.** — Habrá de ser unatro día.
- **Konforme, X-(e)an geratuko gara.** — D'alcuerdo, quedamos o X.

## Repaso visual

```
Pasau    →  Presente  →  Futuro
atzo         gaur          bihar — etzi — datorren astean
joan zen     doa            joango da
ikusi dut    ikusten dut    ikusiko dut
```

> **Nota cultural:** en muitos puestos veyerás cartels de l'estilo *"Asteburu honetan jaiak"* o *"Datorren ostiralean kontzertua"*. Os marcadors temporals como *datorren* y *asteburu honetan* amaneixen tanto en a conversación como en qualsiquier panel informativo d'o barrio. Cuan a orella los reconoixe, leyer a carrera se fa muito mas fácil.
