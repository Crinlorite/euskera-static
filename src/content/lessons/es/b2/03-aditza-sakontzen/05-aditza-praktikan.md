---
id: b2-aditza-5
unitId: b2-aditza
code: 05-aditza-praktikan
unit: 03-aditza-sakontzen
level: b2
order: 5
title: El verbo en uso real
estimatedMinutes: 12
covers: [b2-indicative-review, b2-perpaus-osagarriak, b2-zehar-estiloa-tense, b2-denbora-nean]
exercises:
  - id: ex-b2-adi5-mc1
    type: multiple-choice
    prompt: '"Me ha dicho que cuando llegue a Bilbao, te avisará" — ¿qué subordinada de tiempo cabe?'
    options:
      - "Bilbora iristen denean"
      - "Bilbora iristerakoan"
      - "Bilbora iritsitakoan"
      - "Cualquiera de las tres"
    answer: 3
    explanation: 'Las tres encajan: el matiz cambia (genérico, en el momento de llegar, una vez allí), pero gramaticalmente son válidas.'
  - id: ex-b2-adi5-mc2
    type: multiple-choice
    prompt: 'Identifica la frase con más errores:'
    options:
      - "Esan dit etortzen denean deituko nauela"
      - "Esan dit etortzen denean deituko didala"
      - "Esan zidan etorri zenean deituko zidala"
      - "Galdetu zidan non bizi nintzen"
    answer: 0
    explanation: '*Deitu* en sentido "llamar por teléfono" pide dativo (NOR-NORI-NORK): *deituko didala* ("me llamará a mí"). *Deituko nauela* sería "me llamará en el sentido de me invocará / me requerirá", incorrecto en este contexto.'
  - id: ex-b2-adi5-fb1
    type: fill-in-blank
    prompt: '"Sabíamos que se había olvidado del libro" — Bagenekien liburua ahaztu ___________.'
    answers: [zitzaiola, ZITZAIOLA]
    explanation: '*Ahaztu* en sentido involuntario es NOR-NORI: pasado *zitzaion* + completiva *-la* = *zitzaiola*.'
  - id: ex-b2-adi5-fc
    type: flashcards
    cards:
      - { eu: esan zidan etorriko zela, es: me dijo que vendría }
      - { eu: galdetu zidan ondo nengoen, es: me preguntó si estaba bien }
      - { eu: gomendatu zidan jateko, es: me recomendó comer }
      - { eu: agindu zidan etortzeko, es: me ordenó venir }
      - { eu: iruditu zitzaion polita zela, es: le pareció que era bonito }
      - { eu: ahaztu zitzaion deitzea, es: se le olvidó llamar }
      - { eu: gertatu zitzaion bidean, es: le ocurrió por el camino }
      - { eu: itxaron diot etortzeko, es: he esperado a que viniera }
  - id: ex-b2-adi5-mp
    type: match-pairs
    pairs:
      - { eu: deitu dit, es: me ha llamado (a mí) }
      - { eu: deitu nau, es: me ha llamado / convocado }
      - { eu: ekarri dit, es: me lo ha traído }
      - { eu: ekarri nau, es: me ha traído (a mí mismo/a) }
      - { eu: ahaztu zait, es: se me ha olvidado }
---

En esta lección integramos lo aprendido en las cuatro anteriores: **régimen verbal**, **completivas**, **estilo indirecto** y **subordinadas de tiempo**. La diferencia entre A2 y B2 está en saber **combinarlos** sin dudar.

## Decisiones que tomas en cada frase

Cuando vayas a producir una frase de cierto nivel, internamente decides:

1. **¿Qué auxiliar pide el verbo?** (DA / DU / NOR-NORI / NOR-NORI-NORK)
2. **¿Tiempo y aspecto?** (presente / pasado / futuro / habitual)
3. **¿Va dentro de una oración mayor?** (completiva, temporal, relativa)
4. **¿Quién dice qué a quién?** (estilo indirecto si reportas)

## Frase compuesta: ejemplo paso a paso

> *Olatzek esan zidan, etxera iristen denean, bere amari deitzen diola beti.*

Vamos al detalle:

| Pieza | Análisis |
|---|---|
| *Olatzek esan zidan* | Verbo introductor, pasado, NOR-NORI-NORK (a mí me dijo Olatz). |
| *etxera iristen denean* | Subordinada temporal con -(e)nean, presente habitual. |
| *bere amari deitzen diola beti* | Completiva con -(e)la sobre auxiliar NOR-NORI-NORK: *bere amari + deitzen + dio + -la*. |

Hay tres niveles de subordinación, dos cambios de sujeto, y un régimen distinto en cada verbo. Esa densidad es lo que pide B2.1.

## Verbos con pareja "yo / a mí"

Hay verbos que cambian completamente de régimen según quién es el sujeto y quién recibe la acción. Memoriza estas parejas:

| Frase | Régimen | Sujeto | Objeto |
|---|---|---|---|
| *Deitu nau* | NOR-NORK | él/ella | yo (acusativo) |
| *Deitu dit* | NOR-NORI-NORK | él/ella | a mí (dativo) + algo (no expreso, "una llamada") |
| *Ekarri nau* | NOR-NORK | él/ella | yo (me trae a mí) |
| *Ekarri dit* | NOR-NORI-NORK | él/ella | algo a mí (me trae algo) |
| *Eraman nau* | NOR-NORK | él/ella | yo (me lleva) |
| *Eraman dit* | NOR-NORI-NORK | él/ella | algo de mí / a mí |

> Cuando la traducción al castellano es "me ha + verbo", **no asumas** que es siempre el mismo régimen. La pregunta es: ¿el "me" es complemento directo (NOR) o indirecto (NORI)?

## Combinar zehar estiloa con subordinada temporal

Esta combinación es típica de B2 escrito:

Estilo directo:

> Olatzek atzo: *"Bilbora iritsitakoan, deituko dizut."*

Estilo indirecto, introductor en pasado:

> Olatzek atzo esan zidan, **Bilbora iritsitakoan**, deituko **zidala**.

Cambios:

1. *Iritsitakoan* — la subordinada temporal **no cambia**: ya no tiene tiempo propio, lo toma del contexto.
2. *Deituko dizut* (futuro presente, "te llamaré") → *deituko zidala* (futuro pasado, "me llamaría").

## Frases-tipo para integrar

Memoriza estas como **plantillas**:

- *Esan zidan ___ zela.* — Me dijo que era ___.
- *Galdetu zidan ___ nintzen.* — Me preguntó si yo estaba ___.
- *Iritsi nintzenean, ___ zegoen.* — Cuando llegué, había ___.
- *Bukatu ondoren, ___ egingo dugu.* — Después de terminar, haremos ___.
- *Gomendatu dit ___ -tzeko.* — Me ha recomendado ___.

Estas estructuras, encadenadas, te permiten producir párrafos de informe, narración o argumentación sin pensar en la mecánica.

## Ejercicio mental para fijar

Coge una conversación cualquiera y conviértela en **informe** en pasado. Por ejemplo:

> **Conversación:**
> — *Garazi: "Bihar Bilbora joango naiz."*
> — *Lander: "Zein ordutan?"*
> — *Garazi: "Goizeko zortzietan."*

> **Informe:**
> *Garazik esan zion Landerri hurrengo egunean Bilbora joango zela. Landerrek galdetu zion zein ordutan, eta Garazik erantzun zion goizeko zortzietan joango zela.*

Tres frases que comprimen toda la mecánica del estilo indirecto + completivas.

## Errores que te delatan en B2

| Error | Forma correcta |
|---|---|
| *Esan dit etortzen* (sin -la) | *Esan dit etortzen **dela***. |
| *Galdetu dit non bizi naiz* (sin -en) | *Galdetu dit non bizi **naizen***. |
| *Ahaztu dut etortzea* (régimen mal) | *Ahaztu **zait** etortzea*. |
| *Iritsi nintzen denean* (mal lugar de -en) | *Iritsi **nintzenean***. |
| *Esan zuela* (sin sujeto principal) | *Esan **zuen** etorriko **zela***. |

## Plan de trabajo para asentar B2 verbal

1. **Una semana**, frase indirecta diaria: convierte una frase oída en castellano a indirecto en euskera.
2. **Otra semana**, una temporal por día: *cuando llegue, al llegar, una vez llegado, después de llegar* — todas con el mismo verbo.
3. **Tercera semana**, mezcla: zehar estiloa + temporal + completiva en una misma frase.

Tres semanas después, el verbo deja de ser un obstáculo y vuelve a ser solo una pieza que conecta el resto.
