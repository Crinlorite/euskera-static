---
id: a1-week-plan-5
unitId: a1-week-plan
code: 05-nire-astea
unit: 13-agenda
level: a1
order: 5
title: Mi semana (lección integradora)
estimatedMinutes: 12
covers: [weekly-planning, future-koGo, time-when, days-of-week, common-verbs]
exercises:
  - id: ex-na-mc1
    type: multiple-choice
    prompt: '"Mañana iré al gimnasio" =…'
    options: ['Bihar kiroldegira joan naiz', 'Bihar kiroldegira joango naiz', 'Bihar kiroldegian nago', 'Kiroldegira bihar dut']
    answer: 1
    explanation: 'Futuro: *joan + go = joango* + *naiz* (yo). *Kiroldegira* = al gimnasio (sufijo -ra de destino).'
  - id: ex-na-mc2
    type: multiple-choice
    prompt: '"El sábado quedaré con Maialen para tomar café" =…'
    options: ['Larunbatean Maialenekin geratuko naiz kafea hartzeko', 'Larunbat Maialen kafea hartu', 'Maialen larunbatean kafea du', 'Larunbatean kafea Maialenekin']
    answer: 0
    explanation: '*Larunbatean* (el sábado) + *Maialenekin* (con Maialen) + *geratuko naiz* (quedaré) + *kafea hartzeko* (para tomar café).'
  - id: ex-na-fb1
    type: fill-in-blank
    prompt: '"La semana que viene tendré exámenes" = ___ astean azterketak izango ditut.'
    answers: [Datorren, datorren]
    explanation: '*Datorren* — "que viene". *Datorren astean* — "la semana que viene".'
  - id: ex-na-fb2
    type: fill-in-blank
    prompt: '"El viernes saldré con los amigos" = Ostiralean lagunekin atera___ naiz.'
    answers: [ko]
    explanation: '*Atera* + *ko* + *naiz* = *aterako naiz* (saldré). Verbo en -a → -ko.'
  - id: ex-na-fc
    type: flashcards
    cards:
      - { eu: agenda, es: agenda }
      - { eu: 'Asteko agenda', es: Agenda de la semana }
      - { eu: 'Zer egingo duzu astelehenean?', es: ¿Qué harás el lunes? }
      - { eu: 'Kiroldegira joango naiz', es: Iré al gimnasio }
      - { eu: 'Lagunekin afaltzera joango gara', es: Iremos a cenar con los amigos }
      - { eu: 'Asteburu honetan mendira joango gara', es: Este fin de semana iremos al monte }
      - { eu: 'Datorren astean lan asko izango dut', es: La semana que viene tendré mucho trabajo }
      - { eu: 'Ostiralean parranda egingo dugu', es: El viernes saldremos de marcha }
      - { eu: 'Bilera dut hamarretan', es: Tengo reunión a las diez }
      - { eu: 'Libre zaude?', es: '¿Estás libre?' }
      - { eu: 'Geratuko gara?', es: '¿Quedamos?' }
  - id: ex-na-mp
    type: match-pairs
    pairs:
      - { eu: 'datorren astean', es: la semana que viene }
      - { eu: 'asteburuan', es: el fin de semana }
      - { eu: 'bihar', es: mañana }
      - { eu: 'etzi', es: pasado mañana }
      - { eu: 'gaur arratsaldean', es: esta tarde }
      - { eu: 'libre nago', es: estoy libre }
---

Lección final de la unidad: vamos a montar **una semana entera** uniendo todo lo aprendido. Días, futuros, citas, marcadores de cuándo... el lenguaje de planificar.

## Pregunta abierta

> *Zer egingo duzu aste honetan?* — ¿Qué vas a hacer esta semana?

Una respuesta natural usaría el día + futuro:

> *Astelehenean kiroldegira joango naiz. Asteartean euskara klasea dut. Asteazkenean bilera bat izango dut. Ostegunean lasai egongo naiz. Ostiralean lagunekin afaltzera joango gara. Asteburuan mendira joango gara.*
>
> El lunes iré al gimnasio. El martes tengo clase de euskera. El miércoles tendré una reunión. El jueves estaré tranquilo. El viernes iremos a cenar con los amigos. El fin de semana iremos al monte.

## Plantilla básica

> **DÍA + ACTIVIDAD + FUTURO**

| Día | Actividad ejemplo |
|---|---|
| *Astelehenean...* | *...kiroldegira joango naiz* |
| *Asteartean...* | *...lan egingo dut* |
| *Asteazkenean...* | *...amarekin geratuko naiz* |
| *Ostegunean...* | *...filma bat ikusiko dut* |
| *Ostiralean...* | *...lagunekin aterako naiz* |
| *Larunbatean...* | *...mendira joango gara* |
| *Igandean...* | *...etxean lasai egongo naiz* |

## Verbos útiles para la agenda

| Verbo | Forma futura | Castellano |
|---|---|---|
| joan | joango naiz/da | iré, irá |
| etorri | etorriko naiz/da | vendré, vendrá |
| egon | egongo naiz | estaré |
| izan | izango da | será / habrá |
| ikusi | ikusiko dut | (lo) veré |
| egin | egingo dut | (lo) haré |
| jan | jango dut | (lo) comeré |
| lan egin | lan egingo dut | trabajaré |
| atera | aterako naiz | saldré |
| eraman | eramango dut | (lo) llevaré |
| erosi | erosiko dut | (lo) compraré |

## Mini-historia: la semana de Iratxe

Lee esta agenda inventada y fíjate en los marcadores temporales:

> *Astelehenean lana dut goizean, eta arratsaldean kiroldegira joango naiz. Asteartean medikuarekin daukat hitzordua, hamabietan. Asteazkenean ez dut ezer berezirik, etxean afalduko dut lasai. Ostegunean Maialenekin geratuko naiz kafe bat hartzeko, sei eta erdietan. Ostiralean afari bat dugu lankideekin. Larunbatean asteburu pasa: Bilbora joango gara nire bikotekidearekin. Igandean buelta etxera, eta arratsaldean lasai-lasai filma bat ikusiko dut.*

## Tu turno: tres respuestas modelo

**Pregunta:** *Bihar zer egingo duzu?*

> *Bihar lanera joango naiz goizean. Arratsaldean kafea hartuko dut lagun batekin, eta gauean etxean afalduko dut.*

**Pregunta:** *Datorren astean zer planik duzu?*

> *Datorren astean azterketa bat izango dut. Asko ikasiko dut. Larunbatean lasai gelditu nahi dut.*

**Pregunta:** *Asteburu honetan zer egingo duzu?*

> *Asteburu honetan mendira joango gara lagunekin. Larunbatean Aralarrera joango gara, eta igandean lasai geldituko gara etxean.*

## Frases clave para coordinar la semana

- **Libre zaude X-(e)an?** — ¿Estás libre el X?
- **Hamarretan ondo?** — ¿A las diez bien?
- **Beste egun batean izan beharko du.** — Tendrá que ser otro día.
- **Konforme, X-(e)an geratuko gara.** — De acuerdo, quedamos el X.

## Repaso visual

```
Pasado    →  Presente  →  Futuro
atzo         gaur          bihar — etzi — datorren astean
joan zen     da             izango da
ikusi dut    ikusten dut    ikusiko dut
```

> **Nota cultural:** en muchos sitios verás carteles del estilo *"Asteburu honetan jaiak"* o *"Datorren ostiralean kontzertua"*. Los marcadores temporales como *datorren* y *asteburu honetan* aparecen tanto en conversación como en cualquier panel informativo del barrio. Cuando el oído los reconoce, leer la calle se vuelve mucho más fácil.
