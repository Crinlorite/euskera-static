...
level: a1
order: 5
title: Izan vs egon (calidade vs estado)
estimatedMinutes: 11
covers:
- izan-vs-egon
- izan-nor
...
- id: ex-ive-mc1
  ...
  prompt: Para dicir "Maialen é alegre (en xeral, así é ela)" usamos…
  options:
  - Maialen alai dago
  - Maialen alaia da
  - Maialen pozik dago
  - Maialen alaia dago
  answer: 1
  explanation: Calidade estable → *izan* + adxectivo con `-a`.
- id: ex-ive-mc2
  type: multiple-choice
  prompt: Para dicir "Hoxe Maialen está alegre / contenta (é un estado)" usamos…
  options:
  - Maialen pozik dago
  - Maialen alaia da
  - Maialen pozik da
  - Maialen alaia dago
  answer: 0
  explanation: Estado pasaxeiro → *egon* + palabra sen `-a`. *pozik* é a opción natural.
- id: ex-ive-fb1
  type: fill-in-blank
  prompt: Meu pai é nervioso (é así) = Nire aita urduria ___.
  answers:
  - da
  explanation: 'Calidade → *izan*: *da*.'
- id: ex-ive-fb2
  type: fill-in-blank
  prompt: Hoxe meu pai está nervioso (estado de hoxe) = Gaur nire aita urduri ___.
  answers:
  - dago
  explanation: 'Estado pasaxeiro → *egon*: *dago*. E fíxate en como o adxectivo perde o `-a`.'
- id: ex-ive-fb3
  type: fill-in-blank
  prompt: Estamos cansos = Nekatuta ___.
  answers:
  - gaude
  explanation: Forma de *egon* para "nós" = *gaude*.
- id: ex-ive-fc
  type: flashcards
  cards:
  - eu: Nolakoa zara?
    es: Como es? (calidade)
  - eu: Nola zaude?
    es: Como estás? (estado)
  - eu: alaia naiz
    es: son alegre (sempre)
  - eu: pozik nago
    es: estou contenta (agora)
  - eu: lasaia da
    es: é tranquilo (carácter)
  - eu: lasai dago
    es: está tranquilo (neste momento)
  - eu: urduria naiz
    es: son nerviosa (carácter)
  - eu: urduri nago
    es: estou nerviosa (agora)
  - eu: gaztea da
    es: é novo
  - eu: nekatuta dago
    es: está canso
...
Esta é probablemente a distinción máis escorregadiza do A1 en éuscaro. Como en galego "ser" vs "estar", pero con dúas diferenzas importantes: **(1) os adxectivos cambian de forma** e **(2) algunhas palabras só se usan cun dos dous verbos**.
...
3. **Estou a describir un trazo físico permanente (alto, gordo, novo)?** Vai con *izan* + `-a`.
...
> **Nota:** en galego diríase "son nervioso" ou "estou nervioso" sen cambiar a palabra. En éuscaro a palabra cambia (*urduria* vs *urduri*). Ao principio custa; coa práctica diaria sae só. Se dubidas nunha conversa real, unha saída segura é usar *egon* cunha palabra invariable: *haserre nago*, *triste nago*, *nekatuta nago* — sempre funcionan.
