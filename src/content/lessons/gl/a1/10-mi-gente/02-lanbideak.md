---
id: a1-people-2
unitId: a1-people
code: 02-lanbideak
unit: 10-mi-gente
level: a1
order: 2
title: Lanbideak — Profesións (ampliación)
estimatedMinutes: 12
covers:
- professions
- professions-extended
exercises:
- id: ex-10-02-mp
  type: match-pairs
  pairs:
  - eu: irakaslea
    es: profesor/a
  - eu: ikaslea
    es: estudiante
  - eu: medikua
    es: médico/a
  - eu: erizaina
    es: enfermero/a
  - eu: sukaldaria
    es: cocinero/a
  - eu: kazetaria
    es: periodista
- id: ex-10-02-mc1
  type: multiple-choice
  prompt: Quen é un "polizia"?
  options:
  - Un médico
  - Un policía
  - Un xornalista
  - Un cociñeiro
  answer: 1
  explanation: '"Polizia" = policía. É un préstamo, pero a forma vasca con artigo é "polizia" (sen -a final adicional).'
- id: ex-10-02-mc2
  type: multiple-choice
  prompt: '"Mekanikaria" traballa con...'
  options:
  - enfermos
  - libros
  - coches
  - niños
  answer: 2
  explanation: '"Mekanikaria" é o mecánico, adoita traballar en talleres con coches.'
- id: ex-10-02-fb1
  type: fill-in-blank
  prompt: Son estudante = Ni ___ naiz.
  answers:
  - ikaslea
  explanation: '"Ikaslea" = estudante. Lembra: en éuscaro "son X" dise "Ni X-a naiz", co substantivo + artigo "-a".'
- id: ex-10-02-fc
  type: flashcards
  cards:
  - eu: lanbidea
    es: profesión
  - eu: ikaslea
    es: estudante
  - eu: irakaslea
    es: profesor/a
  - eu: medikua
    es: médico/a
  - eu: erizaina
    es: enfermeiro/a
  - eu: sukaldaria
    es: cociñeiro/a
  - eu: kazetaria
    es: xornalista
  - eu: polizia
    es: policía
  - eu: dendaria
    es: dependente/a
  - eu: zerbitzaria
    es: camareiro/a
  - eu: mekanikaria
    es: mecánico/a
  - eu: idazlea
    es: escritor/a
  - eu: abokatua
    es: avogado/a
  - eu: arkitektoa
    es: arquitecto/a
  - eu: igeltseroa
    es: albanel
  - eu: nekazaria
    es: agricultor/a
  - eu: arrantzalea
    es: pescador/a
  - eu: langilea
    es: traballador/a
  - eu: jubilatua
    es: xubilado/a
  - eu: langabea
    es: en paro / desempregado/a
  - eu: Zer da zure lanbidea?
    es: Cal é a túa profesión?
  - eu: Zertan egiten duzu lan?
    es: En que traballas?
---

Esta lección **amplía** as profesións que xa viches na unidade de descrición persoal. Aquí tes un repertorio maior, con foco nas que aparecen en historias e conversas cotiás sobre veciños.

## Repaso rápido

| Éuscaro | Galego |
|---|---|
| *ikaslea* | estudante |
| *irakaslea* | profesor/a |
| *medikua* | médico/a |
| *langilea* | traballador/a |
| *jubilatua* | xubilado/a |
| *langabea* | en paro |

## Ampliación

| Éuscaro | Galego |
|---|---|
| *erizaina* | enfermeiro/a |
| *sukaldaria* | cociñeiro/a |
| *kazetaria* | xornalista |
| *idazlea* | escritor/a |
| *abokatua* | avogado/a |
| *arkitektoa* | arquitecto/a |
| *polizia* | policía |
| *suhiltzailea* | bombeiro/a |
| *dendaria* | dependente/a |
| *zerbitzaria* | camareiro/a |
| *mekanikaria* | mecánico/a |
| *igeltseroa* | albanel |
| *nekazaria* | agricultor/a |
| *arrantzalea* | pescador/a |
| *gidaria* | condutor/a |
| *administrazioko langilea* | traballador/a da administración |

## Sufixos que delatan a profesión

Se te fixas, moitas profesións levan sufixos típicos:

- **`-ari` / `-lari`** → "o/a que se dedica a…": *kazetari*, *dantzari*, *mekanikari*, *sukaldari*.
- **`-le`** → "o/a que fai…": *irakasle*, *idazle*, *ikasle*, *arrantzale*.
- **`-zain` / `-zai`** → "gardián/a de…": *erizain* (coidador/a de enfermos = enfermeiro/a), *atezaina* (porteiro/a).
- **`-tzaile`** → forma produtiva: *saltzailea* (vendedor/a), *suhiltzailea* (bombeiro/a).

Saber identificar estes sufixos axúdache a **adiviñar** profesións novas sen dicionario.

## "Ser de profesión X"

O patrón xa o coñeces das primeiras unidades:

- **Ni medikua naiz.** — Son médico.
- **Bera kazetaria da.** — El/ela é xornalista.
- **Gu ikasleak gara.** — Nós somos estudantes.

> Importante: para nomear unha profesión en xeral, vai co artigo `-a`: *medikua*, *kazetaria*. (Na fala tamén oirás *mediku naiz*, sen artigo; as dúas formas son válidas, pero neste curso usamos a forma con `-a`.)

## Preguntar pola profesión

| Pregunta | Galego |
|---|---|
| *Zer da zure lanbidea?* | Cal é a túa profesión? |
| *Zertan egiten duzu lan?* | En que traballas? |
| *Non egiten duzu lan?* | Onde traballas? |
| *Zer da bere lana?* | Cal é o seu traballo? |

E para responder sobre un mesmo ou sobre outra persoa:

- *Ikaslea naiz, baina dendari moduan ere lan egiten dut.* — Son estudante, pero tamén traballo de dependente.
- *Bera erizaina da, ospitalean lan egiten du.* — É enfermeiro/a, traballa no hospital.

> **Nota:** en éuscaro, dicir "son enfermeiro" ou "son enfermeira" dise igual: **erizaina naiz**. Non hai marcas de xénero nos substantivos. Acláralo co contexto ou co nome da persoa.
