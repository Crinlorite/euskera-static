---
id: a1-recent-past-1
unitId: a1-recent-past
code: 01-zer-egin-duzu-intro
unit: 08-pasado-reciente
level: a1
order: 1
title: Que fixeches hoxe? — o pasado recente
estimatedMinutes: 10
covers:
- participle-form
- recent-past-dut
- recent-past-naiz
exercises:
- id: ex-01-zedi-mc1
  type: multiple-choice
  prompt: '"Zer egin duzu gaur?" significa…'
  options:
  - Que fas todos os días?
  - Que fixeches hoxe?
  - Que vas facer mañá?
  - Que hora é?
  answer: 1
  explanation: '"Egin" é o partizipio de "facer"; "duzu" é a forma de ukan para "ti". Xuntos = "fixeches".'
- id: ex-01-zedi-mc2
  type: multiple-choice
  prompt: 'Compara: "egunero gosaltzen dut" vs "gaur gosaldu dut". Cal é a diferenza?'
  options:
  - A primeira é presente, a segunda é futuro
  - A primeira é habitual (almorzo cada día), a segunda é pasado recente (almorcei hoxe)
  - A segunda non é correcta
  - As dúas significan o mesmo
  answer: 1
  explanation: 'É a clave: o partizipio + auxiliar SEN -tzen é pasado recente. Con -tzen é habitual.'
- id: ex-01-zedi-fb1
  type: fill-in-blank
  prompt: Comín = jan ___ (auxiliar para "eu + obxecto").
  answers:
  - dut
  explanation: '"Jan" (comer) é transitivo: leva ukan. "Eu" → *dut*. Resultado: *jan dut* (comín).'
- id: ex-01-zedi-fb2
  type: fill-in-blank
  prompt: Fun = joan ___ (auxiliar para "eu" sen obxecto).
  answers:
  - naiz
  explanation: '"Joan" (ir) é intransitivo: leva izan. "Eu" → *naiz*. Resultado: *joan naiz* (fun).'
- id: ex-01-zedi-fc
  type: flashcards
  cards:
  - eu: Zer egin duzu gaur?
    es: Que fixeches hoxe?
  - eu: Zer egin du Mikelek?
    es: Que fixo Mikel?
  - eu: Zer egin duzue gaur?
    es: Que fixestes hoxe?
  - eu: gaur
    es: hoxe
  - eu: gaur goizean
    es: hoxe pola mañá
  - eu: gaur arratsaldean
    es: hoxe pola tarde
  - eu: partizipioa
    es: 'partizipio (forma do verbo: jan, edan, joan, ikusi)'
  - eu: jan dut
    es: comín
  - eu: joan naiz
    es: fun
  - eu: ikusi dut
    es: vin (de ver)
  - eu: etorri naiz
    es: vin (de vir)
  - eu: gosaldu dut
    es: almorcei
  - eu: esnatu naiz
    es: espertei
---

No éuscaro, para contar o que fixeches hai pouco —sobre todo hoxe— úsase o **orainaldi burutua** (literalmente "presente perfecto"), que equivale ao *comín / fun* do galego. É o que se coñece como **pasado recente**.

## A fórmula é moi sinxela

> **Partizipioa + auxiliar (izan ou ukan)**

Comparada co aspecto habitual que viches na unidade anterior, o único que cambia é que o verbo vai **sen -tzen / -ten**:

| Habitual | Pasado recente | Galego |
|---|---|---|
| *gosaltzen dut* | **gosaldu dut** | almorzo → almorcei |
| *jaten dut* | **jan dut** | como → comín |
| *joaten naiz* | **joan naiz** | vou → fun |
| *etortzen naiz* | **etorri naiz** | veño → vin |
| *ikusten dut* | **ikusi dut** | vexo → vin |

> **É a mesma diferenza que en galego** entre presente habitual e pretérito perfecto:
>
> - *Egunero euskaltegira **joaten naiz**.* — Cada día vou ao euskaltegi.
> - *Gaur goizean euskaltegira **joan naiz**.* — Esta mañá fun ao euskaltegi.

## A pregunta máis natural

> *— **Zer egin duzu gaur?*** — Que fixeches hoxe?
>
> *— Lan egin dut, eta gero zinera joan naiz.* — Traballei, e despois fun ao cine.

Fíxate nas dúas formas: **lan egin dut** (con *dut* porque "traballar" leva obxecto en éuscaro) e **joan naiz** (con *naiz* porque "ir" non leva obxecto).

## Os dous auxiliares — repaso rápido

### Con izan (NOR) — naiz / zara / da / gara / zarete / dira

| Pronome | Auxiliar | Exemplo |
|---|---|---|
| ni (eu) | naiz | *joan **naiz*** (fun) |
| zu (ti) | zara | *etorri **zara*** (viñeches) |
| hura (el/ela) | da | *esnatu **da*** (espertou) |
| gu (nós) | gara | *joan **gara*** (fomos) |
| zuek (vós) | zarete | *etorri **zarete*** (viñestes) |
| haiek (eles) | dira | *joan **dira*** (foron) |

### Con ukan (NOR-NORK) — dut / duzu / du / dugu / duzue / dute

| Pronome | Auxiliar | Exemplo |
|---|---|---|
| nik (eu) | dut | *jan **dut*** (comín) |
| zuk (ti) | duzu | *ikusi **duzu*** (viches) |
| hark (el/ela) | du | *erosi **du*** (mercou) |
| guk (nós) | dugu | *egin **dugu*** (fixemos) |
| zuek (vós) | duzue | *jan **duzue*** (comestes) |
| haiek (eles) | dute | *ikusi **dute*** (viron) |

## Mini-conversa

> *— Kaixo Maialen! Zer moduz?*
>
> *— Ondo. Eta zu? **Zer egin duzu gaur?***
>
> *— Lan egin dut goizean, eta arratsaldean lagun batekin kafea hartu dut.*
>
> *— Polita. Ni gaur ez naiz lanera joan, gaixo egon naiz.*

> **En resumo:** todo o truco está en saber que auxiliar usa cada verbo (verémolo nas próximas dúas leccións) e manter o partizipio sen a marca habitual *-tzen/-ten*.
