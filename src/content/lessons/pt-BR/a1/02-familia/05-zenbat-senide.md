---
id: a1-family-5
unitId: a1-family
code: 05-zenbat-senide
unit: 02-familia
level: a1
order: 5
title: Quantos irmãos você tem? (Zenbat senide?)
estimatedMinutes: 12
covers:
- ukan-basic
- ukan-plural-objects
- partitive-rik
- numbers-1-20
- ages
exercises:
- id: ex-zs-fb1
  type: fill-in-blank
  prompt: Tenho dois irmãos = Bi anaia ___.
  answers:
  - ditut
  explanation: '"Ditut" = "eu tenho (vários)" — forma de "ukan" (ter) NOR-NORK.'
- id: ex-zs-mc1
  type: multiple-choice
  prompt: Como se diz "três" em euskera?
  options:
  - bat
  - bi
  - hiru
  - lau
  answer: 2
- id: ex-zs-fc
  type: flashcards
  cards:
  - eu: bat
    es: um
  - eu: bi
    es: dois
  - eu: hiru
    es: três
  - eu: lau
    es: quatro
  - eu: bost
    es: cinco
  - eu: hamar
    es: dez
  - eu: Zenbat senide zarete?
    es: Quantos irmãos vocês são?
  - eu: senide
    es: irmão/irmã (genérico)
  - eu: dut
    es: tenho (1 coisa)
  - eu: ditut
    es: tenho (várias coisas)
  - eu: ez dut anaiarik
    es: não tenho irmãos (negação com -rik)
  - eu: Zenbat urte dituzu?
    es: Quantos anos você tem?
---

Para falar sobre quantos membros uma família tem, usamos:

- **Zenbat senide zarete?** → "Quantos irmãos vocês são?" (pergunta-se no plural: o total de irmãos, incluindo você)
- **Familia handi/txiki bat dut.** → "Tenho uma família grande/pequena."

## Números de 1 a 10

| Número | Euskera |
|---|---|
| 1 | *bat* |
| 2 | *bi* |
| 3 | *hiru* |
| 4 | *lau* |
| 5 | *bost* |
| 6 | *sei* |
| 7 | *zazpi* |
| 8 | *zortzi* |
| 9 | *bederatzi* |
| 10 | *hamar* |

## Como dizer quantos você tem

Aqui entra uma primeira forma do verbo **ukan** (ter), que veremos a fundo nas próximas lições:

- **Anaia bat dut.** — Tenho um irmão.
- **Bi anaia ditut.** — Tenho dois irmãos.
- **Hiru seme ditut.** — Tenho três filhos.
- **Ez dut anaiarik.** — Não tenho irmãos. (forma negativa)

> **dut** é usado quando o que você tem é **uma só coisa** ou um substantivo no singular. **ditut** é usado quando são **várias coisas**. É como a diferença entre "tenho um livro" (singular) e "tenho livros" (plural).

## Frases típicas em apresentações

- *Familia handi bat dut: bi anaia eta arreba bat.* — Tenho uma família grande: dois irmãos e uma irmã.
- *Senide bakarra naiz.* — Sou filho único.
- *Zure familia handia da?* — Sua família é grande?

## Idade

- **Zenbat urte dituzu?** — Quantos anos você tem?
- **Hogeita hamar urte ditut.** — Tenho trinta anos.
- **Hirurogeita hamabost urte ditu.** — Ele/Ela tem setenta e cinco anos.
