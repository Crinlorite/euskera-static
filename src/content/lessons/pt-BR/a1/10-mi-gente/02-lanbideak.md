---
id: a1-people-2
unitId: a1-people
code: 02-lanbideak
unit: 10-mi-gente
level: a1
order: 2
title: Lanbideak — Profissões (ampliação)
estimatedMinutes: 12
covers:
- professions
- professions-extended
exercises:
- id: ex-10-02-mp
  type: match-pairs
  pairs:
  - eu: irakaslea
    es: profesor/a
  - eu: ikaslea
    es: estudiante
  - eu: medikua
    es: médico(a)
  - eu: erizaina
    es: enfermero/a
  - eu: sukaldaria
    es: cocinero/a
  - eu: kazetaria
    es: periodista
- id: ex-10-02-mc1
  type: multiple-choice
  prompt: Quem é um "polizia"?
  options:
  - Um médico
  - Um policial
  - Um jornalista
  - Um cozinheiro
  answer: 1
  explanation: '"Polizia" = policial. É um empréstimo, mas a forma basca com artigo é "polizia" (sem -a final adicional).'
- id: ex-10-02-mc2
  type: multiple-choice
  prompt: '"Mekanikaria" trabalha com...'
  options:
  - enfermos
  - libros
  - coches
  - niños
  answer: 2
  explanation: '"Mekanikaria" é o mecânico, costuma trabalhar em oficinas com carros.'
- id: ex-10-02-fb1
  type: fill-in-blank
  prompt: Sou estudante = Ni ___ naiz.
  answers:
  - ikaslea
  explanation: '"Ikaslea" = estudante. Lembre-se: em basco "sou X" se diz "Ni X-a naiz", com o substantivo + artigo "-a".'
- id: ex-10-02-fc
  type: flashcards
  cards:
  - eu: lanbidea
    es: profissão
  - eu: ikaslea
    es: estudante
  - eu: irakaslea
    es: professor(a)
  - eu: medikua
    es: médico(a)
  - eu: erizaina
    es: enfermeiro(a)
  - eu: sukaldaria
    es: cozinheiro(a)
  - eu: kazetaria
    es: jornalista
  - eu: polizia
    es: policial
  - eu: dendaria
    es: vendedor(a)
  - eu: zerbitzaria
    es: garçom/garçonete
  - eu: mekanikaria
    es: mecânico(a)
  - eu: idazlea
    es: escritor(a)
  - eu: abokatua
    es: advogado(a)
  - eu: arkitektoa
    es: arquiteto(a)
  - eu: igeltseroa
    es: pedreiro
  - eu: nekazaria
    es: agricultor(a)
  - eu: arrantzalea
    es: pescador(a)
  - eu: langilea
    es: trabalhador(a)
  - eu: jubilatua
    es: aposentado(a)
  - eu: langabea
    es: desempregado(a)
  - eu: Zer da zure lanbidea?
    es: Qual é a sua profissão?
  - eu: Zertan egiten duzu lan?
    es: Em que você trabalha?
---

Esta lição **amplia** as profissões que você já viu na unidade de descrição pessoal. Aqui você tem um repertório maior, com foco nas que aparecem em histórias e conversas cotidianas sobre vizinhos.

## Revisão rápida

| Basco | Português |
|---|---|
| *ikaslea* | estudante |
| *irakaslea* | professor(a) |
| *medikua* | médico(a) |
| *langilea* | trabalhador(a) |
| *jubilatua* | aposentado(a) |
| *langabea* | desempregado(a) |

## Ampliação

| Basco | Português |
|---|---|
| *erizaina* | enfermeiro(a) |
| *sukaldaria* | cozinheiro(a) |
| *kazetaria* | jornalista |
| *idazlea* | escritor(a) |
| *abokatua* | advogado(a) |
| *arkitektoa* | arquiteto(a) |
| *polizia* | policial |
| *suhiltzailea* | bombeiro(a) |
| *dendaria* | vendedor(a) |
| *zerbitzaria* | garçom/garçonete |
| *mekanikaria* | mecânico(a) |
| *igeltseroa* | pedreiro |
| *nekazaria* | agricultor(a) |
| *arrantzalea* | pescador(a) |
| *gidaria* | motorista |
| *administrazioko langilea* | trabalhador(a) administrativo(a) |

## Sufixos que revelam a profissão

Se você prestar atenção, muitas profissões têm sufixos típicos:

- **`-ari` / `-lari`** → "aquele/aquela que se dedica a…": *kazetari*, *dantzari*, *mekanikari*, *sukaldari*.
- **`-le`** → "aquele/aquela que faz…": *irakasle*, *idazle*, *ikasle*, *arrantzale*.
- **`-zain` / `-zai`** → "guardião/guardiã de…": *erizain* (cuidador(a) de doentes = enfermeiro(a)), *atezaina* (porteiro(a)).
- **`-tzaile`** → forma produtiva: *saltzailea* (vendedor(a)), *suhiltzailea* (bombeiro(a)).

Saber identificar esses sufixos ajuda você a **adivinhar** profissões novas sem dicionário.

## "Ser de profissão X"

O padrão você já conhece das primeiras unidades:

- **Ni medikua naiz.** — Sou médico.
- **Bera kazetaria da.** — Ele/ela é jornalista.
- **Gu ikasleak gara.** — Nós somos estudantes.

> Importante: para nomear uma profissão de forma geral, usa-se o artigo `-a`: *medikua*, *kazetaria*. (Na fala você também ouvirá *mediku naiz*, sem artigo; as duas formas são válidas, mas neste curso usamos a forma com `-a`.)

## Perguntar sobre a profissão

| Pergunta | Português |
|---|---|
| *Zer da zure lanbidea?* | Qual é a sua profissão? |
| *Zertan egiten duzu lan?* | Em que você trabalha? |
| *Non egiten duzu lan?* | Onde você trabalha? |
| *Zer da bere lana?* | Qual é o trabalho dele(a)? |

E para responder sobre si mesmo ou sobre outra pessoa:

- *Ikaslea naiz, baina dendari moduan ere lan egiten dut.* — Sou estudante, mas também trabalho como vendedor.
- *Bera erizaina da, ospitalean lan egiten du.* — É enfermeiro(a), trabalha no hospital.

> **Nota:** em basco, dizer "sou enfermeiro" ou "sou enfermeira" se diz da mesma forma: **erizaina naiz**. Não há marcas de gênero nos substantivos. Você esclarece com o contexto ou com o nome da pessoa.
